...
export async function POST(req: NextRequest) {
  try {
    ...
    const backendUrl = process.env.OAUTH_BACKEND_URL;
    if (!backendUrl) {
      return NextResponse.json(
        { error: 'Authentication service not configured', code: 'OAUTH_NOT_CONFIGURED' },
        { status: 503 }
      );
    }

    const authRes = await fetch(`${backendUrl}/auth/v1/env/oauth/session-data`, {
    ...
    try {
      ...
      return NextResponse.json(
        { error: 'Database unavailable', details: dbErr instanceof Error ? dbErr.message : 'Connection failed', code: 'DB_UNAVAILABLE' },
        { status: 503 }
      );
    }
    ...
    console.error('[api/auth/session] Unhandled error:', err);
    return NextResponse.json(
