...
export async function GET(req: NextRequest) {
  try {
    ...
    console.error('[api/passkeys] GET Error:', err);
    return NextResponse.json(
      { error: 'Database unavailable', details: err instanceof Error ? err.message : 'Connection failed', code: 'DB_UNAVAILABLE', data: [] },
      { status: 503 }
    );
  }
}
...
export async function DELETE(req: NextRequest) {
  try {
    ...
    console.error('[api/passkeys] DELETE Error:', err);
    return NextResponse.json(
